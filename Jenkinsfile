...
def upgradeRelease(Map<String, String> overwrites) {
    ...
    sh "helm upgrade --wait --install --atomic --timeout 500s --namespace=${NAMESPACE} ${DOCKER_IMAGE} ${HELM_CHART_REGISTRY_NAME}/${HELM_CHART_NAME} --version ${params.helmChartVersion}  -f ./k8s/values-${params.envName}.yaml --debug --force" + arguments
}
